...
let set_of_list xs = List.fold_right Ints.add xs Ints.empty

let build_set_of_ints list_of_strs =
  List.map int_of_string list_of_strs |> set_of_list

let find_pairs_of_sum sum set =
  let check_if_mem remaining = Ints.mem remaining set in
  Ints.filter (fun x -> check_if_mem (sum - x)) set |> Ints.elements

let find_triplets_of_sum sum set =
  let get_all_possible_pairs x = find_pairs_of_sum (sum - x) set in
...
let run_01 =
  ...
  |> build_set_of_ints |> find_pairs_of_sum 2020
  |> List.iter (print_val_and_compliment 2020)

let run_02 =
  read_file "../inputs/day01.txt"
  |> build_set_of_ints |> find_triplets_of_sum 2020
