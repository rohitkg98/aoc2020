...
let next_index curr length right =
  (curr + right) mod length
  
let skip_n n xs =
  List.filteri (fun index _ -> index mod n = 0) xs

let transform trees (right, down) =
  let length = List.hd trees |> String.length in
  let acc (index, count) row =
    (next_index index length right, count + int_of_tree row.[index]) in
  skip_n down trees
  |> List.fold_left acc (0, 0)
